    ...
    <div className="space-y-5 animate-fade-up">
        ...
        <div className="flex flex-col justify-between rounded-2xl bg-primary-dark p-6 text-white shadow-[0_12px_30px_rgba(0,62,93,0.15)]">
          <div>
            ...
            <p className="mt-2 text-[13px] leading-relaxed text-white/80">
              We retain the file order and prepare a single review package for annotation.
            </p>
          </div>
          <div className="mt-7 flex items-center gap-2 border-t border-white/15 pt-4 text-[12px] font-medium text-white/90">
            <ShieldCheck size={16} className="text-secondary" /> PDF plan sets only
          </div>
        </div>
      </div>
      ...
      {hasFiles && (
        <Card className="space-y-3.5">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-[12px] font-semibold uppercase tracking-[0.08em] text-slate-soft">
                Review package
              </p>
            ...
            {allUploaded && (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-forest-soft px-2.5 py-1 text-[11.5px] font-semibold text-forest">
          ...
          <ul className="space-y-2">
            {files.map((file) => (
              <li
                key={file.id}
                className="flex items-center gap-3 rounded-xl border border-paper-line bg-paper/60 px-4 py-3 transition-colors hover:border-primary/30"
              >
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary-soft">
                  {file.status === "error" ? (
                    <AlertTriangle size={17} className="text-alert" />
                  ) : (
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="shrink-0 text-[12px] text-slate-soft">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-paper-line bg-paper-raised px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
        <p className="text-[12.5px] text-slate">
          {allUploaded
            ? "Your files are ready. Continue when you are set."
            : anyUploading
              ? "Uploading files and preparing the review package..."
              : "Add at least one PDF plan set to continue."}
        </p>
        <Button size="lg" disabled={!allUploaded || anyUploading} onClick={onProceed}>
